...
The probe message that nodes in the senders list send are 10 random characters 'Far far aw' (it is the beginning of a passage)  and the window size is always 5. When the last character is received (in timeout), the routing table of the sender's node is updated (if there is change) and it will then send this table to all neighbors. The total number of probe messages sent and numbers dropped is kept as a tally so that the loss rate becomes more and more accurate with more messages sent. To start sending probe messages, probe receivers must first tell the probe senders the link loss rate in an initial contact message. This is included as part of the initial routing table information when the probe receiver is first turned on (when it first received a message and then sending out its routing table information for the first time). The initial loss rate of a particular link (-p value for SR protocol) is embedded in the dictionary as the value for a key -1. 

Parsing logic when receiving distance vectors for neighbors is more involved than in part 3. In particular, we must have logic for receiving initial loss rate from receiver as a sender, and for receiving link cost rate from sender as a receiver. As a receiver, when we receive link cost from the sender, we report this link as the one we use for the sender if it is either the lowest cost, or if we have no other choice. Furthermore, when the link cost is updated as a receiver, any paths that depend on the updated link must also be updated. We update this by iterating through a dictionary (next_hop) with key: node and value: next hop for key and updating the routing table value of those nodes whose next hop is the sender whose link was just updated. 

This part of the assignment is working as intended. The only issue is if the link cost is told to be 0. Then, because we cannot differentiate between no probe packets sent (according to PA instructions) there may be errors in calculation. This is not a problem even if link cost is input as very low (e.g 0.05) because over time the probe messages will converge towards normality after many messages
